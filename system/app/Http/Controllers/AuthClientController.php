...
class AuthController extends Controller
{
	...
	function loginProcess(){
		...
		if(Auth::attempt(['email' => request('email'), 'password' => request('password')])){
			$user = Auth::user();
			if($user->level == 0) return redirect('main/admin')->with('success', 'Login Berhasil');
			if($user->level == 1) return redirect('main/penjual')->with('success', 'Login Berhasil');
			if($user->level == 2) return redirect('main/pembeli')->with('success', 'Login Berhasil');
		}else{
			return back()->with('danger', 'Login Gagal, Silahkan Cek Email dan Password Anda');
		}
